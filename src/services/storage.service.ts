...
export class StorageService {
  ...
  getUsuarioWeb():UsuarioWeb{
    let usr = localStorage.getItem(STORAGE_KEYS.userWeb);
    if (usr == null) {
      return null;
    }
    else {
  ...
  setUsuarioWeb(obj: UsuarioWeb) {
    if (obj == null) {
      localStorage.removeItem(STORAGE_KEYS.userWeb);
    }
    else {
      localStorage.setItem(STORAGE_KEYS.userWeb, JSON.stringify(obj));
    }
  }
