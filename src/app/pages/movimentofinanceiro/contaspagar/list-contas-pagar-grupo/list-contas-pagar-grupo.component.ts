...
import { ContasPagarDtoReport } from 'src/models/movimento/contas-pagar-dto-report';

@Component({
  selector: 'app-list-contas-pagar-grupo',
  templateUrl: './list-contas-pagar-grupo.component.html',
  styleUrls: ['./list-contas-pagar-grupo.component.css']
})
export class ListContasPagarGrupoComponent implements OnInit {
  customIntMaskConfig = {
    align: "right",
    allowNegative: true,
    allowZero: true,
  ...
  empresa: Empresa;
  listcontaspagadtos: ContasPagarDtoReport[];
  ...
  ngOnInit(): void {
    ...
    this.time = setInterval(async () => {
      this.isrealy = false;
      try {
        this.servicegeral.getAll(`${this.controller}/listacontaspagarsintetico`)
          .then(
            (rest) => {
              console.log(rest);
